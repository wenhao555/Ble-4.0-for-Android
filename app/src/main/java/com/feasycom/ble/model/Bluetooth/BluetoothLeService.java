...
package com.feasycom.ble.model.Bluetooth;
...
public class BluetoothLeService extends Service {
    ...
    private String mBluetoothDeviceAddress;   //得到的设备地址
    ...
    public final static String EXTRA_DATA =
            "com.example.bluetooth.le.EXTRA_DATA";   //数据
    ...
    private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
        ...
        public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
            ...
            String intentAction;
            if (newState == BluetoothProfile.STATE_CONNECTED) {//当状态为
    ...
    private void broadcastUpdate(final String action,
        ...
        if (UUID_HEART_RATE_MEASUREMENT.equals(characteristic.getUuid())) {//通道得到的UUID
    ...
    public boolean connect(final String address) {
        ...
        final BluetoothDevice device = mBluetoothAdapter.getRemoteDevice(address);//得到远程蓝牙设备
        if (device == null) {//当device为空的时候  显示设备没有找到
            MyLog.w(TAG, "Device not found.  Unable to connect.");
            return false;
        }
        // We want to directly connect to the device, so we are setting the autoConnect
        // parameter to false.
        mBluetoothGatt = device.connectGatt(this, false, mGattCallback);//连接协议
        MyLog.d(TAG, "Trying to create a new connection.");
        mBluetoothDeviceAddress = address;//得到连接设备的地址
        mConnectionState = STATE_CONNECTING;
        return true;
    }
    ...
    public boolean writeCharacteristic(BluetoothGattCharacteristic characteristic) {
        ...
        return mBluetoothGatt.writeCharacteristic(characteristic);//characteristic
    }
    ...
    public void setCharacteristicNotification(BluetoothGattCharacteristic characteristic,
        ...
        mBluetoothGatt.setCharacteristicNotification(characteristic, enabled);//通道的通知
        // This is specific to Heart Rate Measurement.描述符
        BluetoothGattDescriptor descriptor = characteristic.getDescriptor(
                UUID.fromString(SampleGattAttributes.CLIENT_CHARACTERISTIC_CONFIG));
        //新增修改
        if (descriptor != null) {
            descriptor.setValue(BluetoothGattDescriptor.ENABLE_NOTIFICATION_VALUE);
            mBluetoothGatt.writeDescriptor(descriptor);//发送描述词
        }
    }
     ...
     */
    public List<BluetoothGattService> getSupportedGattServices() {//将所有的协议福放到一个集合中
